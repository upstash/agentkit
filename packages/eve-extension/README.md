...
Start from an eve project (eve ≥ 0.52.0 — the prebuilt extension is built with eve 0.52.1 and its compatibility manifest requires tool contract v29 (dynamicTool v28, hook v20), which eve 0.52.0 is the first release to support; the package declares this as its `eve` peer range), then:
