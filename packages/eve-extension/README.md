...
Start from an eve project (eve ≥ 0.65.0 — the prebuilt extension is built with eve 0.65.0 and its compatibility manifest requires tool contract v55 (dynamicTool v52, hook v25), which eve 0.65.0 is the first release to support; the package declares this as its `eve` peer range), then:
