...
Start from an eve project (eve ≥ 0.52.2 — the prebuilt extension is built with eve 0.52.2 and its compatibility manifest requires tool contract v30 (dynamicTool v29, hook v20), which eve 0.52.2 is the first release to support; the package declares this as its `eve` peer range), then:
