...
Start from an eve project (eve ≥ 0.52.3 — the prebuilt extension is built with eve 0.52.3 and its compatibility manifest requires tool contract v32 (dynamicTool v31, hook v22), which eve 0.52.3 is the first release to support; the package declares this as its `eve` peer range), then:
