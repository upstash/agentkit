...
Start from an eve project (eve ≥ 0.51.1 — the prebuilt extension is built with eve 0.51.1 and its compatibility manifest requires tool contract v27 (dynamicTool v27, hook v20), which eve 0.51.1 is the first release to support; the package declares this as its `eve` peer range), then:
