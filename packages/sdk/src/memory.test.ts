...
describe.skipIf(!hasRedisCreds)("AgentMemory (live Redis)", () => {
  ...
  it("stores and fuzzily recalls memories", async () => {
    await memory.add("The user loves hiking in the mountains", { userId: "recall" });
    await memory.add("The user is allergic to peanuts", { userId: "recall" });
    await memory.searchIndex.waitIndexing();

    const recalled = await memory.recall("hiking mountains", { userId: "recall", topK: 1 });
  ...
  it("tolerates typos via fuzzy matching", async () => {
    await memory.add("The user prefers TypeScript", { userId: "typo" });
    await memory.searchIndex.waitIndexing();
    const recalled = await memory.recall("typescrpt", { userId: "typo", topK: 1 });
    expect(recalled[0]?.text).toContain("TypeScript");
  });

  it("isolates memories by userId", async () => {
    await memory.add("alice likes green tea", { userId: "alice" });
    await memory.add("bob likes black coffee", { userId: "bob" });
    await memory.searchIndex.waitIndexing();

    const aliceHits = await memory.recall("likes drink", { userId: "alice", topK: 5 });
  ...
  it("respects minScore", async () => {
    await memory.add("content about serverless databases", { userId: "score" });
    await memory.searchIndex.waitIndexing();
    const hits = await memory.recall("serverless databases", { userId: "score", minScore: 1e9 });
    expect(hits).toHaveLength(0);
  });

  it("recalls everything for a user when no query is given", async () => {
    await memory.add("first noteless memory", { userId: "all" });
    await memory.add("second noteless memory", { userId: "all" });
    await memory.searchIndex.waitIndexing();

    // No query → filter-only fetch; minScore is ignored, so a high floor still returns them.
    const hits = await memory.recall(undefined, { userId: "all", topK: 10, minScore: 1e9 });
    expect(hits.length).toBeGreaterThanOrEqual(2);
    expect(hits.every((h) => h.text.includes("noteless"))).toBe(true);
    // Scoped: another user sees none of them.
    expect(await memory.recall(undefined, { userId: "all-other", topK: 10 })).toHaveLength(0);
  });

  it("falls back to everything when a query matches nothing", async () => {
    await memory.add("the user lives in Berlin", { userId: "fb" });
    await memory.searchIndex.waitIndexing();
    // A query that won't fuzzily match still returns the user's memories (no empty result).
    const hits = await memory.recall("zzqqxx nonexistent topic", { userId: "fb", topK: 10 });
    expect(hits.some((h) => h.text.includes("Berlin"))).toBe(true);
  });

  it("forgets a memory", async () => {
    const rec = await memory.add("ephemeral note to forget", { userId: "forget" });
    await memory.searchIndex.waitIndexing();
    expect(await memory.recall("ephemeral note", { userId: "forget", topK: 5 })).not.toHaveLength(
      0,
    );

    await memory.forget(rec.id, { userId: "forget" });
    await memory.searchIndex.waitIndexing();
    expect(await memory.recall("ephemeral note", { userId: "forget", topK: 5 })).toHaveLength(0);
  });
  ...
  it("requires a non-empty userId on add / recall / forget", async () => {
    await expect(memory.add("x", { userId: "" })).rejects.toThrow(/userId/i);
    await expect(memory.add("x", { userId: undefined as unknown as string })).rejects.toThrow(
      /userId/i,
    );
    await expect(memory.recall("q", { userId: "" })).rejects.toThrow(/userId/i);
    await expect(memory.recall(undefined, { userId: "" })).rejects.toThrow(/userId/i);
  ...
  it("preserves custom metadata and createdAt round-trip", async () => {
    ...
    await memory.searchIndex.waitIndexing();
    const [hit] = await memory.recall("fact metadata", { userId: "meta", topK: 1 });
