...
// Storage for eve's native memory slots (`agent/memory/*.ts`) lives at
// "@upstash/agentkit-eve/memory": `redisDocuments()`, a backend for eve's own `fileMemory()`. That
// entry point needs eve >= 0.45.2; the tools above have no such floor, which is why it is a
// separate subpath.
