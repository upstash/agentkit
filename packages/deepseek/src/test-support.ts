/**
 * Test-only helpers (not part of the published surface — never imported by `index.ts`).
 *
 * Per the project's testing policy, this package is tested against a real Upstash Redis instance
 * rather than a mock. Credentials are read from the repo-root `.env` (or the environment). When
 * they are absent, `hasRedisCreds` is false and the suites skip themselves so CI without secrets
 * stays green.
 */
...
/** A collision-proof key prefix so parallel test runs never share keys. */
