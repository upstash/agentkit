 ...
 * ---
 *
 * VENDORED, VERBATIM, from the DeepSeek Harness (MIT):
 * `packages/session/session-persistence/tests/contract.ts` @ 0.1.0-rc.5.
 * https://github.com/deepseek-ai/deepseek-harness
 *
 * This is the authoritative conformance suite the first-party JSONL and SQLite
 * backends are held to, and running it unchanged is the point — it is what
 * proves the Redis backend has the SAME semantics rather than merely similar
 * ones. It is copied because the harness ships only `lib/` to npm, so the suite
 * cannot be imported from the published package.
 *
 * Only the `SessionPersistence` type import was repointed at the published
 * package. Do not "improve" anything else here; re-copy it when upgrading the
 * `@deepseek-ai/dsh-session-persistence` peer instead.
 *
